...
const Project = ({ project, isSelected, onClick }) => {
    return (
        <div
            className={`relative border-2 border-dotted border-green-600 bg-black p-4 rounded-lg shadow-md transition duration-300 retro-card 
                        ${isSelected ? 'w-full md:w-full col-span-1 md:col-span-2 lg:col-span-3' : 'col-span-1'}`}
            onClick={onClick}
        >
            ...
            {isSelected ? (
                ...
                <Image src={project.images[0]} alt={project.title} className="w-full h-auto cursor-pointer" width={600} height={600} />
            )}
        </div>
    );
};
